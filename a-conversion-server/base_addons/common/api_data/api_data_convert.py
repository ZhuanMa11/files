                ...
                if type(eval(v)) in [bool, str, bytes, datetime.datetime, datetime.date]:
                    ...
                    if eval(v).id:
                        ...
                        if 'name' in eval(v):
                            return_info[value.replace('_id', '_name') if '_id' in value else value + '_name'] = eval(
                                v).name or ''
        if 'file.library' != order._name:
            file_domain = list()
            file_domain.append(('res_model', '=', order._name))
            file_domain.append(('res_id', '=', order.id))
            files = pool['business.file.res'].sudo().search(file_domain, order='id desc')
            file_list = list()
            for rev in files:
                value = {
                    "id": rev.file_id.id or '',
                    "name": rev.file_id.name or '',
                    "view_path": rev.file_id.view_path or '',
                    "download_path": eval(rev.file_id.download_path) if rev.file_id.download_path else '',
                }
