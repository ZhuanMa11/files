...
from dtcloud.exceptions import AccessDenied, AccessError
from dtcloud.http import _request_stack

import dtcloud.tools
from dtcloud.tests import common
from dtcloud.service import common as auth, model
from dtcloud.tools import DotDict
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, dtcloud.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
