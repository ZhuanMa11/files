...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if dtcloudbot is in a chat with a user or
        if someone pinged dtcloudbot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/dtcloudbot will answer.
        ...
        """
        dtcloudbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if len(record) != 1 or values.get("author_id") == dtcloudbot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=dtcloudbot_id, message_type=message_type, subtype_id=subtype_id)

    def _get_answer(self, record, body, values, command=False):
        # onboarding
        dtcloudbot_state = self.env.user.dtcloudbot_state
        if self._is_bot_in_private_channel(record):
            # main flow
            if dtcloudbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.dtcloudbot_state = "onboarding_command"
                self.env.user.dtcloudbot_failed = False
                return _("Great! 👍<br/>To access special commands, <b>start your sentence with</b> <span class=\"o_dtcloudbot_command\">/</span>. Try getting help.")
            elif dtcloudbot_state == 'onboarding_command' and command == 'help':
                self.env.user.dtcloudbot_state = "onboarding_ping"
                self.env.user.dtcloudbot_failed = False
                return _("Wow you are a natural!<br/>Ping someone with @username to grab their attention. <b>Try to ping me using</b> <span class=\"o_dtcloudbot_command\">@DTCloudBot</span> in a sentence.")
            elif dtcloudbot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.dtcloudbot_state = "onboarding_attachement"
                self.env.user.dtcloudbot_failed = False
                return _("Yep, I am here! 🎉 <br/>Now, try <b>sending an attachment</b>, like a picture of your cute dog...")
            elif dtcloudbot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.dtcloudbot_state = "idle"
                self.env.user.dtcloudbot_failed = False
                return _("I am a simple bot, but if that's a dog, he is the cutest 😊 <br/>Congratulations, you finished this tour. You can now <b>close this chat window</b>. Enjoy discovering DTCloud.")
            elif dtcloudbot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.dtcloudbot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif dtcloudbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or dtcloudbot_state == 'idle':
                return _("Unfortunately, I'm just a bot 😞 I don't understand! If you need help discovering our product, please check "
                         "<a href=\"https://www.dtcloud360.com/page/docs\" target=\"_blank\">our documentation</a> or "
                         "<a href=\"https://www.dtcloud360.com/slides\" target=\"_blank\">our videos</a>.")
            else:
                # repeat question
                if dtcloudbot_state == 'onboarding_emoji':
                    self.env.user.dtcloudbot_failed = True
                    return _("Not exactly. To continue the tour, send an emoji: <b>type</b> <span class=\"o_dtcloudbot_command\">:)</span> and press enter.")
                elif dtcloudbot_state == 'onboarding_attachement':
                    self.env.user.dtcloudbot_failed = True
                    return _("To <b>send an attachment</b>, click on the <i class=\"fa fa-paperclip\" aria-hidden=\"true\"></i> icon and select a file.")
                elif dtcloudbot_state == 'onboarding_command':
                    self.env.user.dtcloudbot_failed = True
                    return _("Not sure what you are doing. Please, type <span class=\"o_dtcloudbot_command\">/</span> and wait for the propositions. Select <span class=\"o_dtcloudbot_command\">help</span> and press enter")
                elif dtcloudbot_state == 'onboarding_ping':
                    self.env.user.dtcloudbot_failed = True
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write <span class=\"o_dtcloudbot_command\">@DTCloudBot</span> and select me.")
                return random.choice([
                    _("I'm not smart enough to answer your question.<br/>To follow my guide, ask: <span class=\"o_dtcloudbot_command\">start the tour</span>."),
                    _("Hmmm..."),
                    _("I'm afraid I don't understand. Sorry!"),
                    _("Sorry I'm sleepy. Or not! Maybe I'm just trying to hide my unawareness of human language...<br/>I can show you features if you write: <span class=\"o_dtcloudbot_command\">start the tour</span>.")
                ])
        return False
    ...
    def _is_bot_pinged(self, values):
        dtcloudbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return dtcloudbot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        dtcloudbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return dtcloudbot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.dtcloudbot_failed
