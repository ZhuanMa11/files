dtcloud.define('sms.sms_widget_tests', function (require) {
"use strict";
...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', async function (assert) {
        ...
        // GSM-7
        await testUtils.fields.editAndTrigger(form.$('.o_input'), "Hello from DTCloud", 'input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 characters, fits in 1 SMS (GSM7) ',
            'Should be "15 characters, fits in 1 SMS (GSM7) " for "Hello from DTCloud"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from DTCloud\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 characters, fits in 1 SMS (GSM7) ',
            'Should be "17 characters, fits in 1 SMS (GSM7) " for "Hello from DTCloud\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from DTCloud").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 characters, fits in 1 SMS (UNICODE) ',
            'Should be "15 characters, fits in 1 SMS (UNICODE) " for "Hêllo from DTCloud"');
        // GSM-7 with 160c
