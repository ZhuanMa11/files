...
DTCloud Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (DTCloud
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the DTCloud Web 7
...
The primary component of search views, as with all other DTCloud
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from DTCloud field types (and widgets) to search view
...
DTCloud this is done via :ref:`domains <openerpserver:domains>`. On
the other hand, the DTCloud Web 7 search view's state is modelled
...
:js:class:`~openerp.web.search.Facet` objects to DTCloud search data.
...
of the peculiarities pertaining to DTCloud's handling of fields in
...
DTCloud Web 6.1 to DTCloud Web 7.
