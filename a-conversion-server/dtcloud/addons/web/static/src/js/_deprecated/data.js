dtcloud.define('web.data', function (require) {
"use strict";
 ...
 * consumed by DTCloud's RPC APIs.
 *
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of DTCloud records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the DTCloud model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // DTCloud ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // DTCloud ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, DTCloud domain
     ...
     * an SQL-like form usable by DTCloud's ORM.
     *
     ...
     * DTCloud's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    ...
    @param {String} name name of the DTCloud model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound DTCloud model.
     *
