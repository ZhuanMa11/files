dtcloud.define('web.ErrorDialogRegistry', function (require) {
"use strict";
...
dtcloud.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    init: function () {
        ...
        this.dtcloudExceptionTitleMap = {
            'dtcloud.addons.base.models.ir_mail_server.MailDeliveryException': _lt("MailDeliveryException"),
            'dtcloud.exceptions.AccessDenied': _lt("Access Denied"),
            'dtcloud.exceptions.AccessError': _lt("Access Error"),
            'dtcloud.exceptions.MissingError': _lt("Missing Record"),
            'dtcloud.exceptions.UserError': _lt("User Error"),
            'dtcloud.exceptions.ValidationError': _lt("Validation Error"),
            'dtcloud.exceptions.Warning': _lt("Warning"),
        };
    ...
    rpc_error: function(error) {
        ...
        // DTCloud custom exception: UserError, AccessError, ...
        if (_.has(this.dtcloudExceptionTitleMap, error.data.name)) {
            error = _.extend({}, error, {
                data: _.extend({}, error.data, {
                    message: error.data.arguments[0],
                    title: this.dtcloudExceptionTitleMap[error.data.name],
                }),
            });
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("DTCloud Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("DTCloud Warning"),
            buttons: [
...
core.crash_registry.add('dtcloud.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("DTCloud Session Expired"),
                message: _t("Your DTCloud session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
...
core.crash_registry.add('dtcloud.http.SessionExpiredException', session_expired);
