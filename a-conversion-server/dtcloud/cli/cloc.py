...
from dtcloud.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    DTCloud cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        dtcloud-bin cloc -p module_path

    Or by providing the name of a database:

        dtcloud-bin cloc --addons-path=dirs -d database
