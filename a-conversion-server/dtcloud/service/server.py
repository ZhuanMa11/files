...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'dtcloud.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The dtcloud_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("dtcloud_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by DTCloud.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(dtcloud, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in dtcloud.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="dtcloud.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from dtcloud.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)  # Steve Reich timing style
            registries = dtcloud.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(dtcloud.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)

            t = threading.Thread(target=target, name="dtcloud.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="dtcloud.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(dtcloud, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = dtcloud.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('dtcloud: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(dtcloud.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The dtcloud_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("dtcloud_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = dtcloud.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('dtcloud.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from dtcloud.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(dtcloud.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            dtcloud.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if dtcloud.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from dtcloud.tests.common import DTCloudSuite
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with dtcloud.api.Environment.manage():
                        load_test_file_py(registry, test_file)

            # run post-install tests
            if config['test_enable']:
                t0 = time.time()
                t0_sql = dtcloud.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             dtcloud.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the dtcloud http server and cron processor.
    """
    global server

    load_server_wide_modules()
    dtcloud.service.wsgi_server._patch_xmlrpc_marshaller()

    if dtcloud.evented:
        server = GeventServer(dtcloud.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(dtcloud.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2 ** 32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which DTCloud is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(dtcloud.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not dtcloud.evented:
        if inotify:
    ...
    if getattr(dtcloud, 'phoenix', False):
        _reexec()
