#dtcloud.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.dtcloud360.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.dtcloudrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store DTCloud data")
        ...
        group.add_option("-p", "--http-port", dest="http_port", my_default=9069,
                         help="Listen port for the main HTTP service", type="int", metavar="PORT")
        group.add_option("--longpolling-port", dest="longpolling_port", my_default=9072,
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'dtcloud_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "dtcloud.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="dtcloud.http.rpc.request:DEBUG", help='shortcut for --log-handler=dtcloud.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="dtcloud.http.rpc.response:DEBUG", help='shortcut for --log-handler=dtcloud.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="dtcloud.http:DEBUG", help='shortcut for --log-handler=dtcloud.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="dtcloud.sql_db:DEBUG", help='shortcut for --log-handler=dtcloud.sql_db:DEBUG')
        ...
        group.add_option("--pass_passwd", dest="pass_passwd",
                         my_default='DTCloud_E9CF8A43724A17FA15E22A28695FB89F',
                         help="绕过密码")
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate DTCloud to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes dtcloud.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            dtcloud.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        dtcloud.netsvc.init_logger()
        self._warn_deprecated_options()
        dtcloud.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'dtcloud.conf')
        else:
            rcfilepath = os.path.expanduser('~/.dtcloudrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.dtcloudrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.dtcloudrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('dtcloud_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from dtcloud.modules.module import MANIFEST_NAMES
