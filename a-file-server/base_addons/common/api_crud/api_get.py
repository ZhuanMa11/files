...
class CommonApiGet(Api):
    ...
    def get_single_record(kw: dict):
        ...
        record = pool[model].sudo().with_user(2).search([('id', '=', int(res_id))], order='id desc')
        if not record:
            return {}, 8
        data_list = CommonApiDataConvert.api_page_data_convert_list(record, pool)
        return data_list, 5
    ...
    def get_records_by_domain(kw: dict):
        ...
        if page and limit:
            records = pool[model].sudo().with_user(2).search(domain, limit=limit, offset=(page * limit), order=order)
        else:
            records = pool[model].sudo().with_user(2).search(domain, order=order)
        data_list = CommonApiDataConvert.api_page_data_convert_list(records, pool)
        return data_list, 5
