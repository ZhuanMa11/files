...
class ParseSdb:
        ...
        def _get_json():
            b = sql3.execute("""
              ...
              (select _objects_eav.id as 'ID',_objects_eav.entity_id as '节点索引',_objects_val.value as '属性的值',_objects_id.external_id as '构建GUID',_objects_attr.name,_objects_attr.category  from _objects_eav 
                   ...
                   inner join _objects_val
                   on _objects_eav.value_id = _objects_val.id
                   where name = 'Component') as component
               inner join
               (select _objects_eav.id as 'ID',_objects_eav.entity_id as '节点索引',_objects_val.value as '属性的值',_objects_id.external_id as '构建GUID',_objects_attr.name,_objects_attr.category  from _objects_eav 
                   inner join _objects_attr
                   on _objects_eav.attribute_id = _objects_attr.id
    ...
    @staticmethod
    def parse_sdb_get_build_properties(guid, sdb_path):
        """
        根据guid获取指定sdb文件的构建属性

        :param guid: 构建id
        :param sdb_path: 指定sdb文件路径
        :return: 构建属性
        """
        sql3 = sqlite3.connect(sdb_path)
        cur = sql3.cursor()
        try:
            a = cur.execute("""
                    select 
                        case _objects_attr.name when 'name' then '名称' else _objects_attr.name end 
                        ,_objects_val.value
                        ,case _objects_attr.category when '__name__' then '名称' else _objects_attr.category end  from _objects_eav
                    inner join _objects_val
                    on _objects_eav.value_id = _objects_val.id
                    inner join _objects_attr
                    on _objects_eav.attribute_id = _objects_attr.id
                    where  _objects_eav.entity_id = (select id from _objects_id where external_id = '{0}' and flags = 0 )
        """.format(guid))
            data_list = list()
            value = dict()
            for info in a.fetchall():
                if info[2] in value:
                    value[info[2]][info[0]] = info[1]
                else:
                    value[info[2]] = {info[0]: info[1]}
    ...
    @staticmethod
    def parse_sdb_get_all_pc_build_properties(where_data, sdb_path,file_id):
        """
        根据查询条件和指定sdb文件查询模型的所有PC构建

        :param where_data: 查询条件
        :param sdb_path: 指定sdb文件路径
        :return:
        """
        sql3 = sqlite3.connect(sdb_path)
        ...
        try:
            a = cur.execute("""
            select T1.external_id as 'guid',T2.value as 'component_code',T1.value as 'component_type',T4.value as 'component_model',T3.value as 'building_level'
            from (
            select _objects_eav.entity_id,_objects_val.value,_objects_attr.name,_objects_id.external_id from  _objects_eav
            inner join _objects_val
            on _objects_eav.value_id = _objects_val.id
            inner join _objects_attr
            on _objects_eav.attribute_id = _objects_attr.id
            and _objects_attr.id = (select id from  _objects_attr where name = '构件类型')
            inner join _objects_id
            on _objects_eav.entity_id = _objects_id.id ) as T1
            left join (
            select _objects_eav.entity_id,_objects_val.value,_objects_attr.name,_objects_id.external_id from  _objects_eav
            inner join _objects_val
            on _objects_eav.value_id = _objects_val.id
            inner join _objects_attr
            on _objects_eav.attribute_id = _objects_attr.id
            and _objects_attr.id = (select id from  _objects_attr where name = '构件编号')
            inner join _objects_id
            on _objects_eav.entity_id = _objects_id.id
            ) as T2
            ...
            inner join _objects_attr
            on _objects_eav.attribute_id = _objects_attr.id
            and _objects_attr.id = (select id from  _objects_attr where name = '楼栋-楼层')
            inner join _objects_id
            on _objects_eav.entity_id = _objects_id.id
            ) as T3
            ...
            inner join _objects_attr
            on _objects_eav.attribute_id = _objects_attr.id
            and _objects_attr.id = (select id from  _objects_attr where name = '构件型号')
            inner join _objects_id
            on _objects_eav.entity_id = _objects_id.id
            ) as T4
            ...
            for info in a.fetchall():
                value = {
                    'component_id': info[0],
                    'component_code': info[1],
                    'component_type': info[2],
                    'component_model': info[3],
                    'building_level': info[4],
                    'file_id': file_id,
                }
