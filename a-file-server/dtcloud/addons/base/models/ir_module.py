...
import dtcloud
from dtcloud import api, fields, models, modules, tools, _
from dtcloud.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from dtcloud.exceptions import AccessDenied, UserError
from dtcloud.osv import expression
from dtcloud.tools.parse_version import parse_version
from dtcloud.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    is_app = fields.Boolean(default=False, string='是否认证',readonly=True)
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'DTCloud Enterprise Edition License v1.0'),
        ('OPL-1', 'DTCloud Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('DTCloud Enterprise Module', default=False)
    ...
    def _button_immediate_function(self, function):
        ...
        try:
            ...
            raise UserError(_("DTCloud is currently processing a scheduled action.\n"
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = dtcloud.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from DTCloud Apps', module_name)
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/dtcloud/addons directory to the new "dtcloud" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'dtcloud' subdirectory ?
                ...
                _logger.info('Copy downloaded module `dtcloud` to `%s`', server_dir)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                dtcloud.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.dtcloud360.com/apps')
