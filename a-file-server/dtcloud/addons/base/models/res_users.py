...
from dtcloud.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from dtcloud.exceptions import AccessDenied, AccessError, UserError, ValidationError
from dtcloud.http import request
from dtcloud.modules.module import get_module_resource
from dtcloud.osv import expression
from dtcloud.service.db import check_super
from dtcloud.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    """ User class. A res.users record models an DTCloud user and is different
        from an employee.
    ...
    def _get_default_image(self):
        """ Get a default image when the user is created without image
            ...
            https://github.com/dtcloud/dtcloud/blob/11.0/dtcloud/addons/base/res/res_partner.py
        """
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by DTCloud (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: dtcloud.exceptions.AccessDenied when old password is wrong
        :raise: dtcloud.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If Your DTCloud is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "DTCloud in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.dtcloud360.com/documentation/14.0/administration/deployment/deploy.html#https for details.",
                    source
                )
