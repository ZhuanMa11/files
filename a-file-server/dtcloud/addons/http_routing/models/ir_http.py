...
import dtcloud
from dtcloud import api, models, registry, exceptions, tools
from dtcloud.addons.base.models.ir_http import RequestUID, ModelConverter
from dtcloud.addons.base.models.qweb import QWebException
from dtcloud.http import request
from dtcloud.osv import expression
from dtcloud.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if dtcloud._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            dtcloud._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if dtcloud._geoip_resolver and request.httprequest.remote_addr:
                record = dtcloud._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except dtcloud.exceptions.MissingError:
