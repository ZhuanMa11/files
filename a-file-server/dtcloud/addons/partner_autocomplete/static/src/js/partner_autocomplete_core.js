dtcloud.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousDTCloud: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var dtcloudSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var dtcloudPromise = self._getDTCloudSuggestions(value, isVAT).then(function (suggestions){
                dtcloudSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with DTCloud result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = dtcloudSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            dtcloudSuggestions.push(suggestion);
                        }
                    });
                }

                dtcloudSuggestions = _.filter(dtcloudSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(dtcloudSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(dtcloudSuggestions);
            };

            self._whenAll([dtcloudPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use DTCloud Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getDTCloudSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousDTCloud.add(def);
    },
    /**
