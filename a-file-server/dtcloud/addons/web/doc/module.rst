...
A very basic DTCloud module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An DTCloud
...
.. note::
    ...
    restart the DTCloud server itself for it to be taken in account.
...
provides: an instance of the DTCloud Web client, with the contents of
...
will require restarting the DTCloud Server to see the changes, as
usual):
...
.. note::
    ...
    will apply only within the confines of the DTCloud Web client) and
...
DTCloud Web's template language is :doc:`qweb`. Although any
...
integration to DTCloud Web widgets.
...
To prevent this irremediable loss, we can use DTCloud's support for
...
basic DTCloud model in which our data will be stored:
...
4. And finally — after restarting DTCloud — navigating to the test
