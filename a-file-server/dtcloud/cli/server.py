...
"""
DTCloud - Server
DTCloud is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - DTCloud SA
"""
...
__author__ = dtcloud.release.author
__version__ = dtcloud.release.version

# Also use the `dtcloud` logger for the main script.
_logger = logging.getLogger('dtcloud')
...
def report_configuration():
    ...
    """
    config = dtcloud.tools.config
    _logger.info("DTCloud version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', dtcloud.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = dtcloud.tools.config
    if not dtcloud.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = dtcloud.modules.registry.Registry.new(dbname)
        with dtcloud.api.Environment.manage():
            with registry.cursor() as cr:
                dtcloud.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with dtcloud.api.Environment.manage():
        with registry.cursor() as cr:
            dtcloud.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    dtcloud.tools.config.parse_config(args)
    check_postgres_user()
    ...
    # 加载数据库的  判断config是否有db_name，有的话创建一个空的数据库，但是如果数据库已经存在就会进入except，不做任何处理
    preload = []
    if config['db_name']:
        preload = config['db_name'].split(',')
        for db_name in preload:
            try:
                dtcloud.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except dtcloud.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        dtcloud.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = dtcloud.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the dtcloud server (default command)"""
    def run(self, args):
        main(args)
