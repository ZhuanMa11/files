...
from dtcloud.exceptions import CacheMiss
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~dtcloud.models.Model.read_group`
    ...
    def mapped(self, records):
        ...
        over a simple call to `~dtcloud.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~dtcloud.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~dtcloud.fields.Float.round()` to round a float with the given precision.
    :func:`~dtcloud.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~dtcloud.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field):
    ...
    :class:`res_currency<dtcloud.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <dtcloud.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-dtcloud: currency may not be already initialized if it is
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record, values=None, validate=True):
        ...
        # the legacy DTCloud convention is to transfer and store binaries
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`dtcloud.tools.image.ImageProcess` for maximum image resolution (default: ``45e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~dtcloud.models.Model._check_company`. Add a default company
