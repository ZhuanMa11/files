...
def is_initialized(cr):
    ...
    """
    return dtcloud.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = dtcloud.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with dtcloud.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())

    for i in dtcloud.modules.get_modules():
        mod_path = dtcloud.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = dtcloud.modules.load_information_from_description_file(i)
...
def has_unaccent(cr):
    ...
    module but any similar function will be picked by DTCloud.
