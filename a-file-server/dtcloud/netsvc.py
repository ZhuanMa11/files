...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('dtcloud.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='dtcloud')
    ...
    # Check that handler.stream has a fileno() method: when running DTCloud
...
DEFAULT_LOG_CONFIGURATION = [
    'dtcloud.http.rpc.request:INFO',
    'dtcloud.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['dtcloud:DEBUG', 'dtcloud.sql_db:INFO', 'dtcloud.http.rpc:DEBUG'],
    'debug_rpc': ['dtcloud:DEBUG', 'dtcloud.sql_db:INFO', 'dtcloud.http.rpc.request:DEBUG'],
    'debug': ['dtcloud:DEBUG', 'dtcloud.sql_db:INFO'],
    'debug_sql': ['dtcloud.sql_db:DEBUG'],
    'info': [],
    'runbot': ['dtcloud:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['dtcloud:WARNING', 'werkzeug:WARNING'],
    'error': ['dtcloud:ERROR', 'werkzeug:ERROR'],
    'critical': ['dtcloud:CRITICAL', 'werkzeug:CRITICAL'],
}
