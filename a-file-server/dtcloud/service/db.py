...
import dtcloud
from dtcloud import SUPERUSER_ID
from dtcloud.exceptions import AccessDenied
import dtcloud.release
import dtcloud.sql_db
import dtcloud.tools
from dtcloud.sql_db import db_connect
from dtcloud.release import version_info
...
def check_super(passwd):
    if passwd and dtcloud.tools.config.verify_admin_password(passwd):
        return True
    raise dtcloud.exceptions.AccessDenied()

# This should be moved to dtcloud.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = dtcloud.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            dtcloud.modules.db.initialize(cr)
            dtcloud.tools.config['load_language'] = lang
            cr.commit()

        registry = dtcloud.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = dtcloud.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = dtcloud.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = dtcloud.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = dtcloud.tools.config['db_template']
    ...
    if dtcloud.tools.config['unaccent']:
        try:
            db = dtcloud.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    dtcloud.sql_db.close_db(db_original_name)
    db = dtcloud.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = dtcloud.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = dtcloud.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = dtcloud.tools.config.filestore(db_original_name)
    to_fs = dtcloud.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    dtcloud.modules.registry.Registry.delete(db_name)
    dtcloud.sql_db.close_db(db_name)
    ...
    fs = dtcloud.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'dtcloud_dump': '1',
        'db_name': cr.dbname,
        'version': dtcloud.release.version,
        'version_info': dtcloud.release.version_info,
        'major_version': dtcloud.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = dtcloud.tools.config.filestore(db_name)
            ...
            dtcloud.tools.exec_pg_command(*cmd)
            if stream:
                dtcloud.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                dtcloud.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = dtcloud.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if dtcloud.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = dtcloud.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = dtcloud.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    dtcloud.modules.registry.Registry.delete(old_name)
    dtcloud.sql_db.close_db(old_name)
    ...
    old_fs = dtcloud.tools.config.filestore(old_name)
    new_fs = dtcloud.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    dtcloud.tools.config.set_admin_password(new_password)
    dtcloud.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        dtcloud.tools.config['update']['base'] = True
        dtcloud.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@dtcloud.tools.mute_logger('dtcloud.sql_db')
...
def list_dbs(force=False):
    if not dtcloud.tools.config['list_db'] and not force:
        raise dtcloud.exceptions.AccessDenied()

    if not dtcloud.tools.config['dbfilter'] and dtcloud.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, DTCloud will not
        ...
        res = sorted(db.strip() for db in dtcloud.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [dtcloud.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of DTCloud
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if dtcloud.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        dtcloud.sql_db.close_db(database_name)
...
def exp_list_lang():
    return dtcloud.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(dtcloud.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return dtcloud.release.version
