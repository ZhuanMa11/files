...
from dtcloud.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = dtcloud.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(dtcloud.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if dtcloud.registry(dbname)._init and not dtcloud.tools.config['test_enable']:
                    raise dtcloud.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    dtcloud.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = dtcloud.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = dtcloud.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with dtcloud.registry(db).cursor() as cr:
