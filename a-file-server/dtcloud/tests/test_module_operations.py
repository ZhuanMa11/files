...
import dtcloud
from dtcloud.tools import topological_sort, unique
from dtcloud.netsvc import init_logger
from dtcloud.tests import standalone_tests
import dtcloud.tests.loader

_logger = logging.getLogger('dtcloud.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with dtcloud.api.Environment.manage():
        with dtcloud.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of DTCloud modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str,
        help="Comma-separated list of paths to directories containing extra DTCloud modules")
...
def test_full(args):
    ...
    with dtcloud.api.Environment.manage():
        with dtcloud.registry(args.database).cursor() as cr:
            env = dtcloud.api.Environment(cr, dtcloud.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = dtcloud.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        dtcloud.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        dtcloud.tools.config['addons_path'] = ','.join([args.addons_path, dtcloud.tools.config['addons_path']])
        if args.data_dir:
            dtcloud.tools.config['data_dir'] = args.data_dir
        dtcloud.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('dtcloud.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('dtcloud.sql_db').setLevel(logging.CRITICAL)
