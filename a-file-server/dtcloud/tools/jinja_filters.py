...
# 手机：13584935775
# 作者：'Amos'
# 公司网址： www.dtcloud.pw  www.100china.cn  http://i.youku.com/amoserp
    ...
    for key in kw.keys():
        if key.find('form-field-boolean-') != -1:
            ...
            if kw[key] == 'False' or kw[key] == '':
                ...
                d = datetime.datetime.strptime(kw[key], '%Y-%m-%d')
            ...
            if kw[key] == 'False' or kw[key] == '':
                ...
                d = datetime.datetime.strptime(kw[key], '%Y-%m-%d %H:%M:%S')
...
def past_time(time):
    """ 返回time距离现在的时间是多久 """
    timestamp = (datetime.datetime.now() - time).total_seconds()
    if timestamp < 60:
        return '刚刚'
...
def _get_models_attachment_list(self,order,kw,limit):
    ...
    for k in attachment:
        ...
        preview_url = ''
        if k.file_path:
            if dtcloud.tools.config.get('preview_url'):
                http = dtcloud.tools.config.get('preview_url')
