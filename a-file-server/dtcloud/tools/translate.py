...
def translate(cr, name, source_type, lang, source=None):
    if source and name:
        ...
        cr.execute('select value from ir_translation where lang=%s and type=%s and name=%s',
                   (lang, source_type, str(name)))
    elif source:
        cr.execute('select value from ir_translation where lang=%s and type=%s and src=%s and md5(src)=md5(%s)',
...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return dtcloud.sql_db.db_connect(db_name)
    ...
    def _get_lang(self, frame):
        ...
        if not lang:
            ...
            if not lang:
                ...
                if cr and uid:
                    env = dtcloud.api.Environment(cr, uid, {})
...
def TranslationFileReader(source, fileformat='po'):
    """ Iterate over translation file to return DTCloud translation entries """
    if fileformat == 'csv':
...
class PoFileReader:
    """ Iterate over po file to return DTCloud translation entries """
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/dtcloud/owl/blob/master/doc/reference/component.md#composition
...
class TranslationModuleReader:
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._lang = lang
        self.env = dtcloud.api.Environment(cr, dtcloud.SUPERUSER_ID, {})
        self._to_translate = []
        self._path_list = [(path, True) for path in dtcloud.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'dtcloud.tools.translate:babel_extract_qweb',
                                                  extra_comments=[WEB_TRANSLATION_COMMENT])
