...
""" DTCloud core library."""


#----------------------------------------------------------
# dtcloud must be a namespace package for dtcloud.addons to become one too
...
import sys
assert sys.version_info > (3, 6), "Outdated python version detected, DTCloud requires Python >= 3.6 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in dtcloud/debian/copyright)
        while 1:
            state = conn.poll()
...
# libc UTC hack
#----------------------------------------------------------
# Make sure the DTCloud server runs in UTC.
import os
...
from . import fields
from . import api
from dtcloud.tools.translate import _, _lt
