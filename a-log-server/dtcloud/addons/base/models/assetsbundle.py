...
from dtcloud import fields, tools, SUPERUSER_ID
from dtcloud.tools.pycompat import to_text
from dtcloud.http import request
from dtcloud.modules.module import get_resource_path
from .qweb import escape
import psycopg2
from dtcloud.tools import func, misc
        ...
        return """
            (function (message) {
                ...
                async function onDOMContentLoaded() {
                    var dtcloud = window.top.dtcloud;
                    if (!dtcloud || !dtcloud.define) {
                        useAlert();
                        return;
                    }
                    ...
                    await new Promise(resolve => {
                        const noLazyTimeout = setTimeout(() => resolve(), 10); // 10 since need to wait for promise resolutions of dtcloud.define
                        dtcloud.define('AssetsBundle.PotentialLazyLoading', function (require) {
                            'use strict';
                    ...
                    var alertTimeout = setTimeout(useAlert, 10); // 10 since need to wait for promise resolutions of dtcloud.define
                    dtcloud.define('AssetsBundle.ErrorMessage', function (require) {
                        'use strict';
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In DTCloud you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
