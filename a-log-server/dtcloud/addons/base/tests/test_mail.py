...
from dtcloud.tests.common import BaseCase, SavepointCase, TransactionCase
from dtcloud.tools import (
...
class EmailConfigCase(SavepointCase):
    @patch.dict("dtcloud.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'dtcloud.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@dtcloud.example.com')
        set_param('mail.catchall.domain', 'dtcloud.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin (admin@test.example.com)" <notification@dtcloud.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@dtcloud.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@dtcloud.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@dtcloud.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@dtcloud.example.com>')
        set_param('mail.catchall.domain', 'dtcloud.example.com')
