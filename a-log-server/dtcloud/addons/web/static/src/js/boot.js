...
(function () {
    ...
    dtcloud.testing = typeof QUnit === 'object';
    dtcloud.remainingJobs = jobs;
    dtcloud.__DEBUG__ = {
    ...
    dtcloud.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__dtcloud_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: dtcloud.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = dtcloud.__DEBUG__.getDependencies(job.name);
            ...
            missing = dtcloud.__DEBUG__.getMissingJobs();
            failed = dtcloud.__DEBUG__.getFailedJobs();
            ...
            if (dtcloud.debug || failed.length || unloaded.length) {
                ...
                if (dtcloud.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        dtcloud.__DEBUG__.jsModules = {
            missing: missing,
    ...
    dtcloud.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            dtcloud.processJobs(jobs, services);
