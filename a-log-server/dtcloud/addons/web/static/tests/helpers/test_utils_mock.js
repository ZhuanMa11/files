dtcloud.define('web.test_utils_mock', function (require) {
"use strict";
...
function _mockGlobalObjects(params) {
    ...
    if ('config' in params) {
        ...
        if ('debug' in params.config) {
            dtcloud.debug = params.config.debug;
        }
    }
 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any DTCloud widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in DTCloudAfterTestHook?
 ...
 *   used to generate mock answers for every kind of routes supported by dtcloud
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught DTCloud
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other DTCloud events bubbling up are intercepted
