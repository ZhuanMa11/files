...
class Shell(Command):
    """Start dtcloud in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        dtcloud.cli.server.report_configuration()
        dtcloud.service.server.start(preload=[], stop=True)
        ...
        with dtcloud.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = dtcloud.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = dtcloud.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
