...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            assert self.__module__.startswith('dtcloud.addons.'), \
                "Invalid import of %s.%s, it should start with 'dtcloud.addons'." % (self.__module__, name)
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for DTCloud models.

    DTCloud models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~dtcloud.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~dtcloud.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~dtcloud.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~dtcloud.models.Model._inherits`-ed models, the inherited field will
    ...
    as attribute.
    """
    _check_worflow_auto = False  # 创建工作流
    ...
    def _add_magic_fields(self):
        ...
        * $CONCURRENCY_CHECK_FIELD is a computed field with its computing
        ...
        # 创建工作流程
        if self._check_worflow_auto == True:
            add('workflow_ids',
                fields.One2many('amos.workflow', 'res_id', domain=lambda self: [('res_model', '=', self._name)],
                                string=u'审批'))
            add('workflow_look', fields.Boolean("审核按钮是否可见", compute='_compute_confirm_uid', help=u'True '))
            add('workflow_confirm_uid', fields.Many2one('res.users', string='当前审批人', compute='_compute_confirm_uid'))
            add('workflow_id', fields.Many2one('amos.workflow', string='准备审核的工作流', compute='_compute_confirm_uid'))
            add('workflow_id', fields.Many2one('amos.workflow', string='准备审核的工作流', compute='_compute_confirm_uid'))
            add('approval_type', fields.Selection([
                ('dtcloud', 'dtcloud'),
                ('wechat', 'wechat')
            ], string='审批类型', default=lambda self: 'wechat' if self.env.user.lang == 'zh_CN' else 'dtcloud'))
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = dtcloud.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or dtcloud.fields.Datetime.now()
    ...
    def _build_model_attributes(cls, pool):
        ...
        for base in reversed(cls.__bases__):
            if not getattr(base, 'pool', None):
                ...
                cls._check_worflow_auto = getattr(base, '_check_worflow_auto', cls._check_worflow_auto)
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], dtcloud.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], dtcloud.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from dtcloud.fields import Char, Integer
    ...
    def _read_group_process_groupby(self, gb, query):
        ...
        if temporal:
            ...
            if tz_convert:
                qualified_field = "timezone('%s', timezone('UTC-08:00',%s))" % (self._context.get('tz', 'UTC-08:00'), qualified_field)
...
%(fields_list)s""") % {
    ...
    def _check_concurrency(self):
        if not (self._log_access and self._context.get(self.CONCURRENCY_CHECK_FIELD)):
            return
        check_clause = "(id = %s AND %s < COALESCE(write_date, create_date, (now() at time zone 'UTC-08:00'))::timestamp)"
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~dtcloud.addons.base.models.res_company`).

        For :class:`~dtcloud.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~dtcloud.fields.Integer`,
          :class:`~dtcloud.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~dtcloud.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~dtcloud.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~dtcloud.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~dtcloud.fields.Date` and
              :class:`~dtcloud.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`dtcloud.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`dtcloud.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~dtcloud.fields.One2many` and
          :class:`~dtcloud.fields.Many2many` use a special "commands" format to
    ...
    def _write(self, vals):
        ...
        if self._log_access:
            ...
            if not vals.get('write_date'):
                columns.append(('write_date', '%s', AsIs("(now() at time zone 'UTC-08:00')")))
    ...
    def _create(self, data_list):
        ...
        if self._log_access:
            ...
            columns0.append(('create_date', "%s", AsIs("(now() at time zone 'UTC-08:00')")))
            columns0.append(('write_uid', "%s", self._uid))
            columns0.append(('write_date', "%s", AsIs("(now() at time zone 'UTC-08:00')")))
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an DTCloud domain.
    ...
    def _compute_confirm_uid(self):
        ...
        for line in self:
            ...
            records = self.env['amos.workflow'].search(domain, order="sequence asc, id asc")

            if len(records._ids) > 1:
                mult_confirm = True  # 如果是隐藏下级审核人
            if records:
                for work in records:
                    workflow_type = work.type
                    if work.type == u'串签':
                        workflow_confirm_uid = work.user_id.id
                        workflow_id = work.id
                        workflow_look = True
                    if work.type in [u'汇签', '并签']:
                        # 当前用户是不是在流程中
                        workflow = records.filtered(lambda t: t.user_id.id == self.env.user.id)
                        if not workflow:
                            workflow_confirm_uid = False
                            workflow_look = False
                            workflow_id = False
                        else:
                            workflow_confirm_uid = self.env.user.id
                            workflow_look = True
    ...
    def workflow_confirm(self):
        ...
        if self.workflow_id:
            ...
            if self.workflow_id.state != u'等待审批' and self.workflow_confirm_uid.id != self._uid:
        ...
        if workflow_type == u'串签':
            domain = [
                ('res_model', '=', self._name),
                ('res_id', '=', self.id),
                ('state', '=', '等待审批'),
            ]
            records = self.env['amos.workflow'].sudo().search(domain, order="sequence asc, id desc")
            ...
            records = self.env['amos.workflow'].search(domain, order="sequence asc, id desc")
            if records:
                if len(records._ids) > 1:
                    mult_confirm = True  # 如果是隐藏下级审核人
                if len(records._ids) <= 1:
        ...
        # ]
        #
        # context['default_form_state'] = context['form_state']
        # context['default_to_state'] = context['to_state']
        # context['default_model'] = self._name
        # context['active_id'] = self.id
        #
        # # TODO(amos): 不同节点的审核流程
        # # 问题跟踪
        # if self._name == 'hd.issue' and self.type_problem in ['队间工作申请', '队内工作申请']:
        #     domain.append(('code', '=', self.type_problem))
        #     context['default_code'] = self.type_problem
        #
        # elif self._name == 'hd.complete.report' and self.is_qc == True:
        #     domain.append(('code', '=', self.is_qc))
        #     context['default_code'] = self.is_qc
        # elif self._name == 'hd.complete.report' and self.is_qc == False:
        #     domain.append(('code', '=', False))
        #     context['default_code'] = False
        # elif self._name == 'hd.tsd' and self.unit_company == True:
        #     domain.append(('code', '=', self.unit_company))
        #     context['default_code'] = self.unit_company
        # elif self._name == 'hd.tsd' and self.unit_company == False:
        #     domain.append(('code', '=', False))
        #     context['default_code'] = False
        # else:
        #     domain.append(('code', '=', ''))
        ...
        # check = self.env['amos.approval.check.user'].sudo().search(domain, order="id desc", limit=1)
        # if check:
        #     res_id = check.id
        #     my_tips = context.get('my_tips')
        #     if self._name == 'hd.issue':
        #         if self.type_problem == '队内工作申请':
        #             my_tips = '请选择部门接口人'
        #
        #     values = {
        #         'name': '',
        #         'is_process_node': context['mult_confirm'],
        #         'state': u'同意',
        #         'my_tips': my_tips,
        #     }
        #     check.write(values)
        ...
        if mult_confirm == False:
            ...
            process = self.env['ir.process'].sudo().search(domain, order="id desc", limit=1)
            if process:
                ...
                if len(line) > 1:
                    ...
                    if line.type == '自定义':
                        ...
                        for group in line.groups_ids:
                            workflow_user_ids.append(group.users)

            else:
                domain = [('model', '=', self._name)]
                process = self.env['ir.process'].sudo().search(domain, order="id desc", limit=1)
                line = process.process_ids.filtered(lambda o: o.state_form == self.state)
                if len(line) > 1:
                    raise UserError(u'警告：系统流程配置错误！')
                else:
        ...
        form_id = self.env.ref('Amos_WorkFlow.view_form_amos_approval_check_user_wizard_01').id
        return {'type': 'ir.actions.act_window',
                'res_model': 'amos.approval.check.user',
                'view_mode': 'form',
                'views': [(form_id, 'form')],
                'target': 'new',
                'res_id': res_id,
    ...
    def open_yes_no(self):
        ...
        #::当前状态如与结束一致 就提醒
        if self.workflow_id:
            if self.workflow_id.end_state == self._context['next_state']:
                raise UserError(u'警告：当前审批流程已发生改变，新刷新当前页面！')
        ...
        if workflow_type == u'串签':
            ...
            if records:
                ...
                if not user:
                    ...
                    workflow_id = records[0].id
                    if records[0].user_id.id != self.env.user.id:
                        raise UserError(u'警告：请等待 %s 审核后' % records[0].user_id.name)

        if workflow_type == u'汇签':
            mult_confirm = False  # 是否多人审核
            last_user = True  # 是否最后一个审核人了
            #:::找当前流程里是不是有我
            domain = [
        ...
        if context.get('stop_flow', False):
            mult_confirm = context['stop_flow']

        context['mult_confirm'] = mult_confirm  # 是不是流程中节点审核
        context['stop_flow'] = context.get('stop_flow', False)  # 是不是最后一道
        context['workflow_id'] = workflow_id  # 准备更新的状态的工作流
        context['last_user'] = last_user  # 是否最后一个审核人了
        context['workflow_type'] = workflow_type  # 工作流类型
        ...
        if mult_confirm == False:
            workflow_user_ids = []  # 查询出的流程节点
            process_user_ids = []  # 固定审核人

            domain = [('model', '=', self._name), ('user_id', '=', self._uid)]
            process = self.env['ir.process'].sudo().search(domain, order="id desc", limit=1)
            if process:
                line = process.process_ids.filtered(
                    lambda o: o.state_form == self.state and o.state_to == context['to_state'])
                if len(line) > 1:
                    raise UserError(u'警告：系统流程配置错误！')
                if line:
                    ...
                    if line.is_department:
                        ...
                        if not workflow_user_ids:
                            raise UserError(u'警告：当前部门:%s 不存在 %s 角色' % (self.department_id.name, str(line.power)))
                    elif line.is_company:
                        domain_power = [('power_id.name', 'in', line.power.split(',')),
                                        ('company_id', '=', self.company_id.id)]
                        workflow_user_ids = self.env['hr.department.power'].sudo().search(domain_power)
                        if not workflow_user_ids:
                            raise UserError(u'警告：当前公司:%s 不存在 %s 角色' % (self.department_id.name, str(line.power)))
                    else:
                        domain_power = [('power_id.name', 'in', line.power.split(','))]
                        workflow_user_ids = self.env['hr.department.power'].sudo().search(domain_power)
                        if not workflow_user_ids:
                            raise UserError(u'警告：当前系统:%s 不存在 %s 角色' % (self.department_id.name, str(line.power)))
            else:
                domain = [('model', '=', self._name)]
                process = self.env['ir.process'].sudo().search(domain, order="id desc", limit=1)
                line = process.process_ids.filtered(
    ...
    def object_ok(self, _context=None):
        """
        设计思想
        用户在界面点击审批或其它按钮，会传递
        ...
        if _context['stop_flow'] == True:
            process = self.env['ir.process.line'].sudo().browse(self._context['process_id'])
            lines.button_workflow_three(lines, process, context, _context)  # TODO(amos): 最后一次审批流程
        else:
            if not self.workflow_id:
                if not context.get('process_id'):
                    ...
                    process = self.env['ir.process.line'].sudo().browse(self._context['process_id'])
                    lines.button_workflow_one(lines, process, context, _context)  # TODO(amos): 第一次工作流
            else:
                process = self.env['ir.process.line'].sudo().browse(self._context['process_id'])
                lines.button_workflow_two(lines, process, context, _context)  # TODO(amos): 流程审批

        #:::更新用户流程 如果 workflow_id = 0 就不执行

        self.env['amos.workflow'].write_workflow(lines, context.get('common_confirm', ''), context)
        ...
        if context.get('messages_partner_ids'):
            ...
            if len(context['messages_partner_ids']) > 0:
                ...
                values = {
                    ...
                    'body': "<strong><font color=\"#FF0000\">%s:</font></strong>\n%s<br/><strong>审批建议/处理意见:</strong>%s" % (
                        '阅知', messages_partner_name, context['message'] or ''),
                ...
                #             # 'ref_menu': 'hd_base.menu_0',  # 顶部菜单
                #         }
                #         self.env.user.add_mail_activity(activity)
        return True

    def button_workflow_one(self, lines, process, context, _context):
        ...
        if _context['is_process_node'] == True and context.get('stop_flow', False) == False:
            ...
            if context['last_user'] == True:
                if process != None:
                    if process.state_form in ['新建']:
                        self.env['amos.workflow'].create_workflow_ok(lines, process.state_form,
            ...
            if hasattr(self, process.button_name):
                ...
                raise UserError(u'警告：[%s] 对象不存在！' % self._context['button_name'])
            ...
            if create_flow == False:
                ...
                records = self.env['amos.workflow'].search(domain, order="sequence asc, id desc", limit=1)
                parent_id = False
                if records:
                    parent_id = records.id
                self.env['amos.workflow'].create_workflow(lines, parent_id, process.next_id.id,
                                                          process.next_id.state_form,
                                                          process.next_id.state_to,
                                                          context)
        return True

    def button_workflow_two(self, lines, process, context, _context):
        create_flow = process.is_stop
        if _context['is_process_node'] == True and context.get('stop_flow', False) == False:
            #:::流程中节点审批
            pass
    ...
    def button_workflow_three(self, lines, process, context, _context):
        create_flow = process.is_stop
        if _context['is_process_node'] == True and context.get('stop_flow', False) == False:
            #:::流程中节点审批
            pass
    ...
    def button_confirm(self, _context=None):
        if not self:
    ...
    def object_no(self, context=None):
        ...
        #:::删除无效的默认值
        self.env['amos.approval.check.user'].sudo().browse(context['workflow_id']).unlink()
        return True
    ...
    def action_home(self):
        ...
        # TODO(amos): 更新工作流
        self.workflow_id.write({'state': u'拒绝', 'note': self._context['message']})
        # TODO(amos): 关闭其它工作流
        user = self.workflow_ids.filtered(lambda t: t.state == u'等待审批')
        user.write({'state': u'已关闭', 'end_date': datetime.datetime.utcnow()})

        state = self._context.get('state', '新建')

        if self.workflow_id.process_id:
            state = self.workflow_id.process_id.refuse
        # 如果不是退回上一级，还需要重启原来的审批流程
        if self.workflow_id.parent_id:
            version_id = datetime.datetime.utcnow()
            version = self.env['amos.workflow'].sudo().browse(self.workflow_id.parent_id.id).version_id
            workflow = self.env['amos.workflow'].sudo().search([('version_id', '=', version)],
            ...
            for v in workflow:
                ...
                self.env['amos.workflow'].create(values).id
    ...
    def Upload_Template(self):
        for record in self:
            ...
            client_action = {'type': 'ir.actions.act_url',
                             ...
                             'target': 'new',
                             'nodestroy': True,
                             'url': '%s' % (context['default_file']),
                             }
            return client_action

    def button_read_mail_activity(self):
        """ 打开指定视图 """
        self.ensure_one()
        context = dict(self._context or {})

        domain = [('res_id', '=', self.id), ('res_model', '=', self._name), ('user_id', '=', self._uid)]
        activity = self.env['mail.activity'].search(domain, order="id desc", limit=1)
        if activity:
            return {
                'name': '我的任务',
                'type': 'ir.actions.act_window',
            ...
            # 警告提示
            self.env.user.notify_warning(message='未找到任务！')

    def button_print(self):
        for record in self:
            ids = record.id
            print_tml = record._name.replace('.', '_') + '.html'
            if 'print_templates' in self._context:
            ...
            if 'active_ids' in self._context:
                ...
                client_action = {'type': 'ir.actions.act_url',
                                 ...
                                 'target': 'new',
                                 'nodestroy': True,
                                 'url': '/Print?model=%s&id=%s&print_tml=%s&context=%s' % (
                                 record._name, ids, print_tml, context),
                                 }
            return client_action

    def _api_test(self, *kw):
    ...
    def _api_chart(self, *kw):
        ...
        self._cr.execute(
            """SELECT  type, SUM(CASE WHEN id is null THEN 0 ELSE 1 END) AS count FROM res_partner GROUP BY type""",
            (self.ids,))
    ...
    def button_go_upload(self):
        #::::判断并生成相关的dtcloud_token
        ...
        dtcloud_token = self.env['dtcloud.token'].sudo().search(
            [('user_id', '=', context.get('uid')), ('name', '=', 'GO_Upload')], order="id desc", limit=1)
        if not dtcloud_token:
            ...
            values = {
                ...
                'token_date': Datetime.now() + relativedelta(hours=+2),
                'date_start': datetime.date.today(),
                'date_end': datetime.date.today() + relativedelta(years=3),
            }
            self.env['dtcloud.token'].sudo().create(values)
        else:
            if dtcloud_token.token_date < Datetime.now():
                access_token = self.token(self.env.user.login, dtcloud_token.appid, dtcloud_token.secret)
                values = {
                    'access_token': access_token,
                    'token_date': Datetime.now() + relativedelta(hours=+10),
                }
                dtcloud_token.sudo().write(values)
            else:
                access_token = dtcloud_token.access_token
        ...
        if res_model == 'documents.document':
            ...
            params.get('url'), res_model, res_id, res_field, company_id, create_uid, write_uid, access_token, name,
            folder_id)
        else:
    ...
    def token(self, login, appid, secret):
        ...
        """
        import hashlib
        API_SECRET = "dtcloud"  # 从接口对接负责人处拿到
    ...
    def ensure_one(self):
        ...
        :raise dtcloud.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~dtcloud.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~dtcloud.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/dtcloud/master-nochange-fp/dtcloud/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`dtcloud.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted DTCloud models.

    DTCloud models are created by inheriting from this class::
...
class TransientModel(Model):
    ...
    def _transient_clean_rows_older_than(self, seconds):
        ...
        query = """
            SELECT id FROM "{}"
            WHERE COALESCE(write_date, create_date, (now() AT TIME ZONE 'UTC-08:00'))::timestamp
                < (now() AT TIME ZONE 'UTC-08:00') - interval %s
        """.format(self._table)
