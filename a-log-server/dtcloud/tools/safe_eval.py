...
DTCloud domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an DTCloud domain expression from an untrusted source.
    ...
    except dtcloud.exceptions.UserError:
        raise
    except dtcloud.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except dtcloud.http.AuthenticationError:
        raise
...
`dtcloud.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `dtcloud.tools.safe_eval`.
""")
    return d
