...
class dtcloud_resolver(etree.Resolver):
    """DTCloud specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (dtcloud.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(dtcloud_resolver(env))
