...
from dtcloud.exceptions import MissingError, UserError, ValidationError, AccessError
from dtcloud.osv import expression
from dtcloud.tools.safe_eval import safe_eval, test_python_expr
from dtcloud.tools.float_utils import float_compare
...
class IrActionsServer(models.Model):
    ...
    Since DTCloud 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: DTCloud Environment on which the action is triggered
#  - model: DTCloud Model of the record on which the action is triggered; is a void recordset
#  - record: record on which the action is triggered; may be void
#  - records: recordset of all records on which the action is triggered in multi-mode; may be void
#  - time, datetime, dateutil, timezone: useful Python libraries
#  - float_compare: DTCloud function to compare floats based on specific precisions
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': dtcloud.exceptions.Warning,
            'UserError': dtcloud.exceptions.UserError,
            # record
            'record': record,
            'records': records,
            # helpers
