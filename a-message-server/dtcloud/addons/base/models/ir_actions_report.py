...
from dtcloud.exceptions import UserError, AccessError
...
from dtcloud.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of DTCloud so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at DTCloud start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start DTCloud with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "DTCloud is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
