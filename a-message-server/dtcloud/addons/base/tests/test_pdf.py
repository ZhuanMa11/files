...
from dtcloud.tests.common import TransactionCase
from dtcloud.tools import pdf
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.DTCloudPdfFileReader(self.minimal_reader_buffer)

    def test_dtcloud_pdf_file_reader(self):
    ...
    def test_dtcloud_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)

        pdf_writer = pdf.DTCloudPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
    ...
    def test_dtcloud_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.DTCloudPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.DTCloudPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.DTCloudPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'DTCloud')
        self.assertEqual(pdf_info['/Creator'], 'DTCloud')
        reader_buffer.close()
