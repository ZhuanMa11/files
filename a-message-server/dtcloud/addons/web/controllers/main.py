...
from dtcloud.api import call_kw, Environment
from dtcloud.modules import get_module_path, get_resource_path
from dtcloud.tools import image_process, topological_sort, html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property, float_repr
from dtcloud.tools.mimetypes import guess_mimetype
from dtcloud.tools.translate import _
from dtcloud.tools.misc import str2bool, xlsxwriter, file_open
from dtcloud.tools.safe_eval import safe_eval, time
from dtcloud import http, tools
from dtcloud.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response
from dtcloud.exceptions import AccessError, UserError, AccessDenied
from dtcloud.models import check_method_name
from dtcloud.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('dtcloud.addons.web', "views")
...
def module_installed_bypass_session(dbname):
    try:
        registry = dtcloud.registry(dbname)
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, dtcloud.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = dtcloud.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("dtcloud.addons.web.main.manifest_list(): debug parameter is deprecated")
    mods = mods.split(',')
...
def fix_view_modes(action):
    """ For historical reasons, DTCloud has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = dtcloud.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return dtcloud.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = dtcloud.tools.config.verify_admin_password('admin')
        d['list_db'] = dtcloud.tools.config['list_db']
        d['langs'] = dtcloud.service.db.exp_list_lang()
        d['countries'] = dtcloud.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = dtcloud.service.db.list_db_incompatible(d['databases'])
        except dtcloud.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = dtcloud.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            dtcloud.service.db.check_super(master_pwd)
            ...
            dump_stream = dtcloud.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=dtcloud.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.dtcloud360.com/oauth2/auth?' + url_encode(params)
    ...
    def _content_image_get_response(
        ...
        if not image_base64:
            ...
            if not (width or height):
                width, height = dtcloud.tools.image_guess_size_from_field_name(field)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = dtcloud.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: dtcloud.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from DTCloud's export data to whatever the
