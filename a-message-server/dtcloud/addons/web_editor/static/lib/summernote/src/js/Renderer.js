define([
  ...
  var Renderer = function () {
    ...
    this.tplButtonInfo = tplButtonInfo; // dtcloud: allow access for override
    this.tplPopovers = tplPopovers; // dtcloud: allow access for override
    ...
    this.createPalette = createPalette; // dtcloud: allow access for override
    ...
    this.createLayoutByAirMode = function ($holder, options) {
      ...
      $holder.addClass('note-air-editor note-editable'); // dtcloud: removing panel-body class to remove unwanted style
      $holder.attr({
        'data-note-id': id, // dtcloud: we use [data-note-id="{id}"] instead of [id="{id}"]
        // 'id': 'note-editor-' + id,
      ...
      // create Popover
      var $popover = $(this.tplPopovers(langInfo, options)); // dtcloud: user (maybe) overrided method
      $popover.addClass('note-air-layout');
      $popover.attr('id', 'note-popover-' + id);
      $popover.appendTo($container);
      createTooltip($popover, keyMap);
      this.createPalette($popover, options); // dtcloud: use (maybe) overrided method
    ...
    this.createLayoutByFrame = function ($holder, options) {
      ...
      //04. create Popover
      var $popover = $(this.tplPopovers(langInfo, options)).prependTo($editingArea); // dtcloud: use (maybe) overrided method
      this.createPalette($popover, options); // dtcloud: use (maybe) overrided method
      createTooltip($popover, keyMap);
      ...
      var keyMap = options.keyMap[agent.isMac ? 'mac' : 'pc'];
      this.createPalette($toolbar, options); // dtcloud: use (maybe) overrided method
    ...
    this.removeLayout = function ($holder, layoutInfo, options) {
      if (options.airMode) {
        $holder.removeClass('note-air-editor note-editable')
               .removeAttr('contentEditable'); // dtcloud: removed id 'id contentEditable'
    ...
    this.getTemplate = function () {
      return {
        ...
        dialog: tplDialog,
        dropdown: tplDropdown // dtcloud: suggest upstream
      };
    };
