dtcloud.define('web_editor.summernote', function (require) {
'use strict';
...
$.summernote.pluginEvents.formatBlock = function (event, editor, layoutInfo, sTagName) {
    ...
    // fix by dtcloud because if you select a style in a li with no p tag all the ul is wrapped by the style tag
    var nodes = dom.listBetween(r.sc, r.ec, r.so, r.eo);
    for (var i=0; i<nodes.length; i++) {
