...
# *broken*
NO_POSTMORTEM = (dtcloud.exceptions.AccessDenied,
                 dtcloud.exceptions.UserError,
                 dtcloud.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                dtcloud.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = dtcloud.service.common.dispatch
        elif service_name == 'db':
            dispatch = dtcloud.service.db.dispatch
        elif service_name == 'object':
            dispatch = dtcloud.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                dtcloud.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                dtcloud.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(dtcloud.tools.exception_to_unicode(e))
        dtcloud.tools.debugger.post_mortem(dtcloud.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all DTCloud Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in dtcloud.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~dtcloud.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~dtcloud.api.Environment` bound to current request. """
        if self._env is None:
            self._env = dtcloud.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            dtcloud.tools.debugger.post_mortem(
                dtcloud.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return dtcloud.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        DTCloud implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <dtcloud.http.WebRequest.csrf_token`, the
          :data:`~dtcloud.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          DTCloud) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (dtcloud.exceptions.UserError,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "DTCloud Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "DTCloud Session Expired"
...
DTCloud URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.dtcloud360.com/documentation/14.0/developer/reference/http.html#csrf for
more details.

* if this endpoint is accessed through DTCloud via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["dtcloud", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('dtcloud.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = dtcloud.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = dtcloud.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ DTCloud provides languages which may not make sense and/or may not
...
dtcloud_DISABLE_SESSION_GC = str2bool(os.environ.get('dtcloud_DISABLE_SESSION_GC', '0'))

if dtcloud_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or dtcloud.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the DTCloud Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = dtcloud.tools.config.session_dir
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in dtcloud.http._generate_routing_rules([''] + dtcloud.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in dtcloud.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        dtcloud.registry(db).check_signaling()
                        with dtcloud.tools.mute_logger('dtcloud.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = dtcloud.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if dtcloud.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = dtcloud.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif dtcloud.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, DTCloud will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in dtcloud.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = dtcloud.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = dtcloud.tools.ustr(filename)
