...
import dtcloud
import dtcloud.modules.db
import dtcloud.modules.graph
import dtcloud.modules.migration
...
_test_logger = logging.getLogger('dtcloud.tests')
...
def force_demo(cr):
    ...
    """
    graph = dtcloud.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    processed_modules = []
    loaded_modules = []
    registry = dtcloud.registry(cr.dbname)
    migrations = dtcloud.modules.migration.MigrationManager(cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = dtcloud.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = dtcloud.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['dtcloud.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = dtcloud.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                ...
                tests_t0, tests_q0 = time.time(), dtcloud.sql_db.sql_counter
                test_results = loader.run_suite(suite, module_name)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = dtcloud.sql_db.sql_counter - tests_q0
        ...
        extra_queries = dtcloud.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   dtcloud.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        ...
        if not dtcloud.modules.db.is_initialized(cr):
        ...
        # dtcloud.modules.registry.Registry.new().
        registry = dtcloud.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['dtcloud.addons.%s' % (pkg.name,)]
                ...
                registry = dtcloud.modules.registry.Registry.new(
