...
import dtcloud
import dtcloud.tools as tools
import dtcloud.release as release
from dtcloud.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__app__.py')
README = ['README.rst', 'README.md', 'README.txt']

_logger = logging.getLogger(__name__)

# addons path as a list
# ad_paths is a deprecated alias, please use dtcloud.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"dtcloud.modules.module.ad_paths" is a deprecated proxy to '
        '"dtcloud.addons.__path__".', DeprecationWarning, stacklevel=2)
    return dtcloud.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "dtcloud.addons".',
    ...
    def load_module(self, name):
        ...
        dtcloud_name = re.sub(r'^openerp.addons.(\w+)$', r'dtcloud.addons.\g<1>', name)

        dtcloud_module = sys.modules.get(dtcloud_name)
        if not dtcloud_module:
            dtcloud_module = importlib.import_module(dtcloud_name)

        sys.modules[name] = dtcloud_module

        return dtcloud_module

class DTCloudHook(object):
    """ Makes dtcloud package also available as openerp """

    def find_module(self, name, path=None):
        # only framework and subdirectories of modules should match
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to dtcloud.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'dtcloud\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `dtcloud.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^dtcloud.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("dtcloud.addons.base.maintenance.migrations", "dtcloud.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``dtcloud.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook dtcloud.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in dtcloud.addons.__path__:
        dtcloud.addons.__path__.append(dd)

    # hook dtcloud.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in dtcloud.addons.__path__:
            dtcloud.addons.__path__.append(ad)

    # hook dtcloud.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in dtcloud.addons.__path__ and os.path.isdir(base_path):
        dtcloud.addons.__path__.append(base_path)

    # hook dtcloud.upgrade on upgrade-path
    from dtcloud import upgrade
    ...
    # create decrecated module alias from dtcloud.addons.base.maintenance.migrations to dtcloud.upgrade
    spec = importlib.machinery.ModuleSpec("dtcloud.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["dtcloud.addons.base.maintenance"] = maintenance_pkg
    sys.modules["dtcloud.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to dtcloud and "crm"-like to dtcloud.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, DTCloudHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in dtcloud.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = dtcloud.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in dtcloud.addons.__path__:
...
def load_openerp_module(module_name):
    """ Load an DTCloud module, if not already loaded.
    ...
    try:
        __import__('dtcloud.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['dtcloud.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in dtcloud.addons.__path__:
