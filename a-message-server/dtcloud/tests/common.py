# -*- coding: utf-8 -*-
"""
The module :mod:`dtcloud.tests.common` provides unittest test cases and a few
...
import dtcloud
from dtcloud import api
from dtcloud.models import BaseModel
from dtcloud.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from dtcloud.service import security
from dtcloud.sql_db import Cursor
from dtcloud.tools import float_compare, single_email_re
from dtcloud.tools.misc import find_in_path
from dtcloud.tools.safe_eval import safe_eval
...
# The dtcloud library is supposed already configured.
ADDONS_PATH = dtcloud.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = dtcloud.SUPERUSER_ID


def get_db_name():
    db = dtcloud.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding DTCloud module name.
    """
    def register(func):
        # register func by dtcloud module name
        if func.__module__.startswith('dtcloud.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most dtcloud operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class DTCloudSuite(unittest.suite.TestSuite):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('dtcloud.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common DTCloud-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.dtcloud360.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~dtcloud.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('dtcloud.sql_db.Cursor.execute', execute):
            with patch('dtcloud.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/dtcloud/addons/" in filename:
                        filename = filename.rsplit("/dtcloud/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = dtcloud.registry(get_db_name())
        ...
        #: :class:`~dtcloud.api.Environment` for the current test case
        self.env = api.Environment(self.cr, dtcloud.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = dtcloud.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, dtcloud.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_dtcloud')
        self.chrome_pid = None

        otc = dtcloud.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way DTCloud widgets are created they all appear as Class(...)
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, dtcloud.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, dtcloud.tools.config['http_port'], url)
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('dtcloud.service.http.request.')]
        ...
        if request_threads:
            ...
            dtcloud.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        dtcloud.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            dtcloud.http.root.session_store.delete(self.session)

        self.session = session  = dtcloud.http.root.session_store.new()
        session.db = db
        ...
        dtcloud.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, dtcloud.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "dtcloud.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "dtcloud.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~dtcloud.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~dtcloud.fields.Many2many` fields, the field itself is a
    :class:`~dtcloud.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~dtcloud.fields.One2many` are reified as
    :class:`~dtcloud.tests.common.O2MProxy`.

    Because the :class:`~dtcloud.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~dtcloud.tests.common.O2MProxy.new` and
    :meth:`~dtcloud.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: dtcloud.models.Model
    ...
    :type view: int | str | dtcloud.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~dtcloud.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~dtcloud.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = dtcloud.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = dtcloud.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return dtcloud.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return dtcloud.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from dtcloud.tests.common have a test_tags
