...
import dtcloud
from dtcloud.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in dtcloud.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [dtcloud.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(dtcloud.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        for a in env['ir.actions.server'].browse(data.keys()):
            self.book(data[a.id] or "dtcloud/studio", "ir.actions.server/%s: %s" % (a.id, a.name), self.parse_py(a.code))
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(data[f.id] or "dtcloud/studio", "ir.model.fields/%s: %s" % (f.id, f.name), self.parse_py(f.compute))
    ...
    def count_database(self, database):
        with dtcloud.api.Environment.manage():
            registry = dtcloud.registry(config['db_name'])
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="DTCloud cloc", lines="Line", other="Other", code="Code")
        s += hr
