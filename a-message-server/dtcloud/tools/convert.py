...
from dtcloud import SUPERUSER_ID, api
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  ...
                  version=dtcloud.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('dtcloud.tools.convert.init').error(
        ...
        # invoke method
        return dtcloud.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def __init__(self, cr, module, idref, mode, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.envs = [dtcloud.api.Environment(cr, SUPERUSER_ID, {})]
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <dtcloud> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['dtcloud', 'data', 'openerp','dtcloud']
