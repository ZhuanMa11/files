...
"""
Miscellaneous tools used by DTCloud.
"""
import cProfile
import collections
import datetime
...
import dtcloud
import dtcloud.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from dtcloud.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if dtcloud.tools.config['db_host']:
        env['PGHOST'] = dtcloud.tools.config['db_host']
    if dtcloud.tools.config['db_port']:
        env['PGPORT'] = str(dtcloud.tools.config['db_port'])
    if dtcloud.tools.config['db_user']:
        env['PGUSER'] = dtcloud.tools.config['db_user']
    if dtcloud.tools.config['db_password']:
        env['PGPASSWORD'] = dtcloud.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the DTCloud root, using a subdir folder.
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the DTCloud root: skip zipfile lookup.
...
def _fileopen(path, mode, basedir, pathinfo, basename=None):
    ...
    paths = dtcloud.addons.__path__ + [config['root_path']]
...
def scan_languages():
    """ Returns all languages supported by DTCloud for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = dtcloud.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating DTCloud domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('dtcloud.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('dtcloud.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = dtcloud.fields.Datetime.from_string(value)
            value = dtcloud.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = dtcloud.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = dtcloud.fields.Datetime.from_string(value)
    else:
        timestamp = value
